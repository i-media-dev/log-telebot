    ...
    def _process_file(self, file_path, project_name):
        try:
            ...
            if tag not in ['SUCCESS', 'ERROR']:
                logging.info(f'Файл {file_path} еще пишется')
                return

            if file_path in self.already_reported:
                logging.info(f'Файл {file_path} уже обработан')
                return
            self.already_reported.add(file_path)
